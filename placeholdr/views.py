...
def show_place(request, place_slug):
	...
	if place_slug:
		...
		if place:
			...
			submitter = str(get_user_prof(place.userId))
...
def submit_trip(request):
	...
	if request.method == "POST":
		...
		if trip_form.is_valid():
			...
			place_slugs = re.findall(pattern, request.POST.get("slug_holder"))
			# There's got to be at least two places in a trip
			if len(place_slugs) > 1:
				...
				t_warning = 'You need to have at least two places in your trip!'
		else:
			print(trip_form.errors, trip_form.errors)

	return render(request, 'placeholdr/submit_trip.html',
				  {'query_string': query_string, 'found': found, 'found_places': found_places, 'trip_form': trip_form, 'warning':t_warning})
...
def ajax_tasks(request):
	...
	if request.method == 'POST':
		...
		if request.POST.get("task") == "add_place_rep" or request.POST.get("task") == "add_trip_rep":
			...
			if str(value) == "0":
				if records.count() > 0:
					return HttpResponse(json.dumps({"exists": True, "rep": records[0].rep}),
			...
			if records.count() > 0:
				...
				UserProfile.objects.filter(id=tpUser.id).update(
					rep=UserProfile.objects.get(id=tpUser.id).rep + int(value))
		...
		if request.POST.get("task") == "trip_search":
			...
			if request.POST.get("q").strip():
				...
				for found_p in found_places:
					results_list.append(
						render(request, 'placeholdr/psearch_result.html', {"place": found_p}).getvalue().decode(
							'utf-8'))
			return HttpResponse(
				json.dumps({'query_string': query_string, 'found': found, 'found_places': results_list}))
		if request.POST.get("task") == "get_added_place":
			return HttpResponse(render(request, 'placeholdr/place_added_bit.html', {
				"place": Place.objects.get(slug=str(request.POST.get("slug").replace("_", "-")))}).getvalue().decode(
				'utf-8'))
